...
#    
    ...
    comparative_best_fitness = alpha*comparative_best_num_vehicles + beta*comparative_best_distance
    comparative_metric_fitness = alpha*comparative_metric_num_vehicles + beta*comparative_metric_distance
...
#            #MUTATION
            new_pop_chromosome_routeList_array, new_pop_chromosome_distanceList_array, arr_new_pop_results = mutation(alpha, beta, N,num_cores, arr_new_pop_results, new_pop_chromosome_routeList_array, new_pop_chromosome_distanceList_array,arr_distance_matrix, arr_customers, total_time, total_capacity, service_time )
...
#           ELITISM
            ...
            #Uncomment this if to run to completion
#            if ((arr_pop_results[np.argmin(arr_pop_results[:,2]),0] ==3) and (np.round(arr_pop_results[np.argmin(arr_pop_results[:,2]),1],2) - 191.81 <0.05)) == True:
#                break
        end_time = time.time() 
        ...
        print(arr_pop_results[np.argmin(arr_pop_results[:,2]),:])
