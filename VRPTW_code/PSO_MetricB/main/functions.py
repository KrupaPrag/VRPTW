...
def learning_probability(particle_i, M, denom, M_1):
#    denom = (math.exp(10)-1)
#    M_1 = M-1
    pC = 0.05 + 0.45*(((math.exp((10*(particle_i-1))/M_1))-1)/denom)
    return pC
#%%
...
def pbest_particle_pC_list(M, denom, M_1):
    ...
    for particle in indicies_false_list:
        arr_pbest_particle[particle] = tournament_selection(particle+1, M)
...
#        print(available_customer_list_index)
        ...
        #calculations:
        arr_nn[:,5] = arr_distance_matrix[curr_loc, available_customer_list] #distance curr next = d_km
        arr_nn[:,6] = arr_nn[:,5] + curr_time #arrival time 
        arr_nn[:,7] = arr_nn[:,2] - arr_nn[:,6] #early - aririval 
        arr_nn[:,7] = arr_nn[:,7].clip(min=0) #waiting time
        arr_nn[:,8]= arr_nn[:,6] + arr_nn[:,7] #execStartTime = arrival + waiting
        arr_nn[:,9] = arr_nn[:,3] - arr_nn[:,8] #r_km = l - t
        arr_nn[:,10] = arr_nn[:,9] + arr_nn[:, 7] + arr_nn[:,5] #nn_km = rkm + waiting m + travel km
...
#        arr_results[flagged_index_val,2] = (np.arctan(PSO_resultlist[3][i])/(math.pi/2)) + arr_results[i,0]#fitness        
        ...
        arr_results[flagged_index_val, 2] = np.arctan(PSO_resultlist[3][i])/(math.pi/2) + arr_results[i,0]
...
#    arr_results[:, 2] = np.arctan(position_resultslist[3])/(math.pi/2)#norm distance
#    arr_results[:,2] = (np.arctan(position_resultslist[3])/(math.pi/2)) + position_resultslist[4]#fitness
    
    arr_results[:,2] = np.arctan(position_resultslist[3])/(math.pi/2) + np.array(position_resultslist[4])#fitness
...
#            print(type(pbest_particle_routeList_list[update_pbest_indicies]))
#            print(type(pop_particle_routeList_list[update_pbest_indicies]))
...
#    6 particle_velocity_list
    ...
    if update_particle_indicies.size !=0: #perform update
        ...
        arr_results[update_particle_indicies, 2] = np.arctan(arr_results[update_particle_indicies, 1])/(math.pi/2)+ (arr_results[update_particle_indicies, 0])#fitness
